...
import { validateCreateCFProjectModel, validateUpdateCFProjectModel } from "../../../../services/ValidationService";
...
const ProjectForm: React.FC<ProjectFormProps> = ({ projectId }) => {
  const [currentProjectDetails, setCurrentProjectDetails] = useState<ReadCFProjectModel>();
  ...
  async function initProjectById() {
    if (projectId) {
      ...
      const projectDetailsResponse: ReadCFProjectModel = await response.json();
      setCurrentProjectDetails(projectDetailsResponse);
    }
  }
    ...
    if (currentProjectDetails) {
      if (projectId) {
        ...
        if (Object.keys(validationErrors).length === 0) {
          const response = await updateCFProjectHttpRequest(projectId, requestBody);
          if (response.ok) {
            showToast("Project has been updated", ToasterMessageType.SUCCESS);
          } else {
            showToast(`Error Status: ${response.status} Error: ${response.text}`);
          }
        }
      } else {
        const validationErrors = validateCreateCFProjectModel(currentProjectDetails);
        setErrors(validationErrors);
        if (Object.keys(validationErrors).length === 0) {
          const response = await createCFProjectHttpRequest(currentProjectDetails as CreateCFProjectModel);
          if (response.ok) {
            showToast("New project created", ToasterMessageType.ERROR);
          } else {
            showToast(`Error Status: ${response.status} Error: ${response.text}`);
          }
        }
        ...
        <form action="" className="project-form">
              ...
              <div className="project-form-input-box four">
                <div className="project-form-input-field">
                  ...
                  <input
                    ...
                    onChange={handleCurrentDetailsChanged}
                  />
                  {errors.fundGoal && <small style={{ color: "red" }}>{errors.fundGoal}</small>}
                </div>
                ...
                <div className="project-form-input-field">
                  ...
                  <input
                    ...
                    onChange={handleCurrentDetailsChanged}
                  />
                  {errors.unitPrice && <small style={{ color: "red" }}>{errors.unitPrice}</small>}
                </div>
                ...
                <div className="project-form-input-field select">
                  ...
                  <select
                    ...
                    onChange={handleCurrentDetailsChanged}
                  >
                    <option value={FundingModel.FIXED_PRICE}>{FundingModel.FIXED_PRICE}</option>
                    <option value={FundingModel.MICRO_INVESTMENT}>{FundingModel.MICRO_INVESTMENT}</option>
                    {errors.fundingModel && <small style={{ color: "red" }}>{errors.fundingModel}</small>}
                  </select>
                </div>
                ...
                <div className="project-form-input-field select">
                  ...
                  >
                    ...
                    <option value={ProjectStatus.PENDING_APPROVAL}>{ProjectStatus.PENDING_APPROVAL}</option>
                    <option value={ProjectStatus.FUNDED}>{ProjectStatus.FUNDED}</option>
              ...
              <div className="project-form-input-box two">
                <div className="project-form-input-field">
                  ...
                  <input
                    ...
                    onChange={handleCurrentDetailsChanged}
                  />
                  {errors.startDate && <small style={{ color: "red" }}>{errors.startDate}</small>}
                </div>
                ...
                <div className="project-form-input-field">
                  ...
                  <input
                    ...
                    onChange={handleCurrentDetailsChanged}
                  />
                  {errors.lastDate && <small style={{ color: "red" }}>{errors.lastDate}</small>}
                </div>
              </div>
            </>
          )}
          ...
          <div className="project-form-input-box one">
            <div className="project-form-input-field button-field">
              <button type="submit" className="project-form-btn" onClick={handleConfirmSaveBtnClick}>
                Confirm Save
              </button>
            </div>
          </div>
